...
| `BtEngineClient` | 协议 `1.1` 协商、配置下发、错误映射和任务快照兼容 |
...
表中的新安装默认值属于 BangumiToday 产品设置。引擎在字段缺省或客户端协议为 `1.0` 时采用安全默认值 `seedingEnabled=false`；`1.1` 客户端在用户确认提示后显式下发产品默认值。

升级自协议 `1.0` 的持久配置时，迁移值为 `seedingEnabled=false`，设置页预填 `2.0` 和 `60`，由用户保存或明确启用后生效。新安装也必须先展示 BT 会暴露 IP 并产生上传流量的说明。
...
`engine.initialize` 和 `engine.configure` 在协议 `1.1` 接受以下增量字段：
...
- 协议 `1.0` 客户端不会发送新增字段，`1.1` 引擎按迁移规则兼容其旧目录；`1.1` 客户端不得假定 `1.0` 引擎支持 `seeding` 状态。
...
| TS-011 | 从协议 `1.0` 配置升级 | 保持完成即停止，直到用户明确启用新策略 |
