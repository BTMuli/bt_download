...
5. 已完成或做种任务强制重校验时，若发现磁盘负载缺失，不得沿用旧停止原因直接回到 `completed`；校验后应恢复下载并重新发现 Peer。
6. `seeding` 任务后台按约 1 秒间隔检查 payload 文件；发现文件被删除或截断时自动进入 `checking` 并重新下载，用户暂停和已停止做种的 `completed` 任务不自动唤醒。
...
| FR-005 | P0 | 任务控制 | 支持暂停、继续、重试、强制校验和移除；强制校验发现已完成任务缺失数据时必须恢复下载；做种中发现 payload 缺失时自动恢复下载；暂停是可持久恢复状态。 |
...
- [ ] 删除做种/已完成任务的负载后执行强制校验，任务能重新下载缺失数据并恢复完整文件。
- [ ] 做种中直接删除或截断 payload 后，任务约 1 秒内自动进入校验并恢复完整文件；用户暂停和已停止做种的任务不会被自动唤醒。
