...
| FR-004 | P0 | 状态机 | 至少覆盖 `metadata`、`checking`、`queued`、`downloading`、`paused`、`completed`、`error`；状态转换可重放且不会因 UI 页面切换而改变。 |
| FR-005 | P0 | 任务控制 | 支持暂停、继续、重试、强制校验和移除；暂停是可持久恢复状态。 |
| FR-006 | P0 | 进度查询 | 快照包含总大小、已完成字节、已验证字节、进度、上下行速度、Peer/Seed 数、保存路径和最后错误。 |
...
| FR-013 | P0 | 默认做种行为 | 下载期间允许受限上传；任务完成后默认停止继续做种。行为应在 UI 中可说明，不能静默无限上传。 |
...
7. 完成通知、打开目录和播放入口仍由 BangumiToday 处理，引擎只发送完成事件。
8. 构建流程先编译/获取固定版本的 `bt_download.exe`，复制到 Flutter Windows Release 目录，再生成 MSIX；CI 校验文件、版本和第三方许可证均存在。
...
| 是否需要长期做种 | P0 完成即停止；P2 再增加按时间/分享率策略。 |
